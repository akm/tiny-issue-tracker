...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
			...
			<TR>
				...
				<th scope="col" class="px-6 py-3">
					<div class="flex items-center">
						Title
						<a
							href={tableHeadLinkSearchParams('title')}
							aria-label="Sort by title"
							onclick={tableHeadLinkClick('title')}
						>
							<SortIcon />
						</a>
					</div>
				...
				<th scope="col" class="px-6 py-3">
					<div class="flex items-center">
						Created At
						<a
							href={tableHeadLinkSearchParams('createdAt')}
							aria-label="Sort by createdAt"
							onclick={tableHeadLinkClick('createdAt')}
						>
							<SortIcon />
						</a>
					</div>
				...
				<th scope="col" class="px-6 py-3">
					<div class="flex items-center">
						Updated At
						<a
							href={tableHeadLinkSearchParams('updatedAt')}
							aria-label="Sort by updatedAt"
							onclick={tableHeadLinkClick('updatedAt')}
						>
							<SortIcon />
						</a>
					</div>
		...
		<tbody>
			{#each items as item}
				<TR>
					...
					<td class="w-6 pr-6 text-right">{item.id}</td>
	...
	<!-- New modal -->
	<FormModal action="?/create" title="New" bind:visible={formModalVisible} submitText="Create">
	...
	<FormModalAlert
		...
		title={'Are you sure you want to delete ' +
			(deletingIssues.length > 1 ? 'these issues' : 'this issue') +
			'?'}
